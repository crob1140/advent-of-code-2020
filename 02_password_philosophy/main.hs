...
splitOneOf delimiters (x:xs) =                        
    ...
    else (x : head rest) : tail rest                  -- Otherwise add this character to the current string
    where
        rest = splitOneOf delimiters xs               -- Use tail recursion to start building our results from the end

parseLine :: String -> (Int, Int, Char, String)
parseLine line = (minAmount, maxAmount, requiredChar, password)
     where
        args = splitOneOf "-: " line
        minAmount = read (args !! 0)                    -- Use read to parse this as an integer
        maxAmount = read (args !! 1)                    -- Use read to parse this as an integer
