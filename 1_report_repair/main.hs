...
findSummingPair :: Int -> [Int] -> Maybe (Int,Int)
findSummingPair sum (x:xs) =
   if elem remainder xs then Just (x, remainder) -- if the remainder exists somewhere in the rest of the list, then we've found our pair
   else findSummingPair sum xs -- otherwise, call this function recursively with the remainder of the list
   where remainder = sum - x
findSummingPair sum [] = Nothing -- if we've run out of elements in our list, then there are no summing pairs

main = case findSummingPair 2020 input of
